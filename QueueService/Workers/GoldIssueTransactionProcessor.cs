...
namespace Goldmint.QueueService.Workers {
	...
	public class GoldIssueTransactionProcessor : BaseWorker {
		...
		public GoldIssueTransactionProcessor(int rowsPerRound) {
		...
		protected override async Task Loop() {
			...
			var rows = await (
				from r in _dbContext.IssueGoldTransaction
				where
			...
			foreach (var row in rows) {
				...
				await CoreLogic.Finance.GoldToken.ProcessIssuingTransaction(_services, row.Id);
			}
		}
	}
}
