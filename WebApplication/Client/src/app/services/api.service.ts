...
import { User, Balance, HistoryRecord, ActivityRecord, OAuthRedirectResponse,
         GoldRate, TFAInfo, KYCStart, KYCStatus, TransparencyRecord, Limits,
         CardsList } from '../interfaces';
import { APIResponse, APIPagedResponse, AuthResponse, RegistrationResponse, CardAddResponse,
         CardConfirmResponse, CardStatusResponse } from '../interfaces/api-response';
...
export class APIService {
  ...
  getHistory(offset: number = 0, limit: number = null,
             sort: string = 'date', order: 'asc'|'desc' = 'desc'): Observable<APIResponse<HistoryRecord[]>> {
    ...
    return this._http
      //@todo: replace by the real api endpoint
      .get(`https://gm-cabinet-dev.pashog.net/api-sandbox.php?action=/api/getHistory`, Object.assign(options, {params: params}))
      .pipe(
        catchError(this._handleError),
        tap(response => {
  ...
  getActivity(offset: number = 0, limit: number = null,
              sort: string = 'date', order: 'asc'|'desc' = 'desc'): Observable<APIPagedResponse<ActivityRecord>> {
